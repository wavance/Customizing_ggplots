...
# There might be multiple linear relationships that may be lost in the noise, co-varites may be sex or species. 
...
# created new graph with the new "color = species" command, to highlight clusters based on species
```{r}
ggplot(data = penguins, aes(x = bill_length_mm, y = bill_depth_mm)) +
...
ggplot(data = penguins, aes(x = bill_length_mm, y = bill_depth_mm)) +
  geom_point(aes(color = species)) +
  scale_color_manual(values = c("green","red","blue"),) #adding manual colors to each species, colors are assigned by aphabetical order
```
...
ggplot(data = penguins, aes(x = bill_length_mm, y = bill_depth_mm)) +
  ...
  scale_color_brewer(palette = "Dark2") +
  scale_shape_manual(values = c(21, 22, 24)) +
  scale_y_continuous(limits = c(0, NA)) #replacing the "30" value with "NA" to set the max y-value to ggplots default max value
```
